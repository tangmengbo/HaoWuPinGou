//
//  CreateHeiDianBaoGuangViewController.m
//  JianZhi
//
...
#import "CreateHeiDianBaoGuangViewController.h"
#import "RAFileManager.h"


@interface CreateHeiDianBaoGuangViewController ()
...
@implementation CreateHeiDianBaoGuangViewController
...
-(void)initTopStepView
{
   ...
   xinXiLeiXingLable.text = @"地址信息";
   [self.mainScrollView addSubview:xinXiLeiXingLable];

   self.addressTF = [[UITextField alloc] initWithFrame:CGRectMake(WIDTH_PingMu-213.5*BiLiWidth, xinXiLeiXingLable.top, 200*BiLiWidth, 39.5*BiLiWidth)];
   self.addressTF.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
   [NormalUse setTextFieldPlaceholder:@"填写地址信息" placeHoldColor:RGBFormUIColor(0xDEDEDE) textField:self.addressTF];
   ...
   [self.mainScrollView addSubview:lineViewXinXi];
    ...
    self.xiangQingTextView.placeholder = @"请对曝光内容进行详细的描述...";
...
-(void)nextButtonClick
{
    ...
    {
        [NormalUse showToastView:@"请填写位置信息" view:self.view];
         return;
    }
    ...
    if (![NormalUse isValidString:self.chanPinShuLiangTF.text]) {
          ...
          [NormalUse showToastView:@"请小姐数量" view:self.view];
          return;
      }
    ...
    if ([NormalUse isValidArray:self.photoPathArray]&& self.photoPathArray.count>0) {
        ...
        [HTTPModel saveFile:[[NSDictionary alloc]initWithObjectsAndKeys:[self.photoPathArray objectAtIndex:uploadImageIndex],@"filepath", nil] callback:^(NSInteger status, id  _Nullable responseObject, NSString * _Nullable msg) {
            ...
            else
            {
                ...
                [dicInfo setObject:self.addressTF.text forKey:@"address"];
                ...
                [HTTPModel faBuHeiDianBaoGuang:dicInfo callback:^(NSInteger status, id  _Nullable responseObject, NSString * _Nullable msg) {
