...
@interface InputMobileViewController ()
...
- (void)viewDidLoad {
    ...
    [wangJiMiMaButton setTitle:@"忘记密码?立即找回" forState:UIControlStateNormal];
    ...
    [HTTPModel mobileAlsoExit:[[NSDictionary alloc]initWithObjectsAndKeys:self.mobileTF.text,@"mobile", nil] callback:^(NSInteger status, id  _Nullable responseObject, NSString * _Nullable msg) {
        ...
        if (status==1) {
            ...
            if (exist_flg.intValue==0) {
                ...
                if([@"2" isEqualToString:self.bangDingOrQieHuan])
                {
                    [NormalUse showToastView:@"当前手机号码未绑定,请更换手机号码" view:self.view];
                }
                else
                {
            ...
            {
                InpurPassWorldViewController * vc = [[InpurPassWorldViewController alloc] init];
                vc.mobileStr = self.mobileTF.text;
                [self.navigationController pushViewController:vc animated:YES];
...
{
    WangJiMiMaInPutMobileViewController * vc = [[WangJiMiMaInPutMobileViewController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
@end
