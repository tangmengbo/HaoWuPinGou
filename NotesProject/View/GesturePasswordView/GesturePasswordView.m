...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        NSInteger margin = (WIDTH_PingMu - 295)/2;
        float ins = 50;
        for (int i = 0; i < 9; i++) {
            NSInteger row = i/3;
            NSInteger col = i%3;
...
{
    [NormalUse defaultsSetObject:password forKey:@"ShouShiPWDefaults"];
    [self.delegate setPassWorldFinsih:password];
}

- (NSString *)getPassword
{
    
    return [NormalUse defaultsGetObjectKey:@"ShouShiPWDefaults"];
