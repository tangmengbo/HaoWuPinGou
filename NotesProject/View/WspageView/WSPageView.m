        ...
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (int i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.height) {
                    ...
                    cell.mainImageView.frame = CGRectMake((cell.bounds.size.width-self.currentWidth*BiLiWidth)/2, cell.bounds.origin.y, self.currentWidth*BiLiWidth, self.currentHeight*BiLiWidth);
                    ...
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(inset, inset, inset, inset));
                    cell.mainImageView.frame = CGRectMake((cell.bounds.size.width-self.normalHeight*self.currentWidth/self.currentHeight*BiLiWidth)/2, cell.bounds.origin.y+(cell.bounds.size.height-self.normalHeight*BiLiWidth)/2, self.normalHeight*self.currentWidth/self.currentHeight*BiLiWidth, self.normalHeight*BiLiWidth);
                    cell.mainImageView.alpha = 1;
...
-(void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    int pageIndex = (int)((int)_scrollView.contentOffset.x / (int)_pageSize.width) % self.orginPageCount;

    [_delegate didScrollToPage:scrollView.contentOffset.x inFlowView:self pageNumber:pageIndex];
}
