...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _mediaArray.count && _mediaArray.count >= _maxImageSelected) {
        [UIAlertController showAlertWithTitle:[NSString stringWithFormat:@"最多只能选择%ld张",(long)_maxImageSelected] message:nil actionTitles:@[@"Confirm"] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
        return;
    }
    ...
    if (indexPath.row == _mediaArray.count) {
        switch (_type) {
            ...
            {
                ACAlertController *alert = [[ACAlertController alloc] initWithActionSheetTitles:@[@"相册", @"相机"] cancelTitle:@"Cancel"];
            ...
            default:
            {
                ACAlertController *alert = [[ACAlertController alloc] initWithActionSheetTitles:@[@"相册", @"相机", @"录像", @"Video"] cancelTitle:@"Cancel"];
                [alert clickActionButton:^(NSInteger index) {
                    if (index == 0) {
                        [weakSelf openAlbum];
                    }else if (index == 1) {
...
- (void)openCamera {
    ...
    if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]){
     ...
     [rootVC presentViewController:picker animated:YES completion:nil];
        ...
        [UIAlertController showAlertWithTitle:@"该设备不支持拍照" message:nil actionTitles:@[@"Confirm"] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
    }
}
...
- (void)openVideotape {
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        [UIAlertController showAlertWithTitle:@"当前设备不支持录像" message:nil actionTitles:@[@"Confirm"] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
    }
